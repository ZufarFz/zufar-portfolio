...
export default function PPTSlideEditor({ slides = [], onUpdateSlides, theme }: PPTSlideEditorProps) {
  ...
  return (
    <div className={`p-5 rounded-xl border space-y-6 ${
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 border-b border-slate-700/30 pb-3">
        ...
        <button
          ...
          className="flex items-center gap-1 bg-emerald-600/20 hover:bg-emerald-650/30 text-emerald-400 border border-emerald-500/30 px-3 py-1 text-[10px] font-bold rounded cursor-pointer transition-all active:scale-97 select-none"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-4 space-y-2 max-h-[350px] overflow-y-auto pr-1">
            ...
            <div
              ...
              className={`p-3 rounded-lg border transition-all cursor-pointer flex items-center justify-between group ${
                sidx === activeIndex
                  ...
                  : theme === 'dark'
                    ? 'bg-slate-950/40 border-slate-800 hover:bg-slate-900 text-slate-350'
                    : 'bg-white border-slate-200 hover:bg-slate-100 text-slate-700'
              }`}
            >
              <div className="flex items-center gap-2.5 min-w-0">
                <span className="text-[10px] font-mono bg-slate-500/15 w-5 h-5 rounded-full flex items-center justify-center shrink-0">
                  {sidx + 1}
                </span>
              ...
              <div className="flex items-center gap-1 shrink-0">
                <span className="text-[8px] font-mono bg-slate-755 px-1 py-0.2 rounded uppercase leading-none opacity-80 group-hover:opacity-100">
                  {slide.visualType}
                </span>
                <button
                  type="button"
          ...
          {slides.length === 0 && (
            <div className="text-center py-8 text-slate-510 font-mono text-xs border border-dashed border-slate-800 rounded-lg">
        ...
        <div className="lg:col-span-8 space-y-4">
          {activeSlide ? (
            <div className={`p-4 rounded-xl space-y-4 ${
              theme === 'dark' ? 'bg-slate-950/40' : 'bg-white border border-slate-200'
            }`}>
              ...
              {/* Slide Title */}
              <div>
                <label className="text-[9px] font-mono font-bold text-slate-400 block uppercase mb-1">Judul Utama Slide PPT</label>
                <input
                  type="text"
                  value={activeSlide.title}
                  onChange={(e) => updateSlideField('title', e.target.value)}
                  className="w-full px-3 py-1.5 bg-slate-900 border border-slate-800 focus:border-emerald-500 rounded-lg text-xs outline-none text-slate-100"
              ...
              {/* Slide Narration */}
              <div>
                <label className="text-[9px] font-mono font-bold text-slate-400 block uppercase mb-1">Narasi Deskripsi Penjelasan Slide</label>
                <textarea
                  value={activeSlide.content}
                  onChange={(e) => updateSlideField('content', e.target.value)}
                  rows={2}
                  className="w-full px-3 py-1.5 bg-slate-900 border border-slate-800 focus:border-emerald-500 rounded-lg text-xs outline-none text-slate-100 leading-normal"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-1">
                ...
                <div>
                  <label className="text-[9px] font-mono font-bold text-slate-400 block uppercase mb-1">Layout Komponen Visual Utama</label>
                  <select
                    value={activeSlide.visualType}
                    onChange={(e) => updateSlideField('visualType', e.target.value)}
                    className="w-full px-3 py-1.5 bg-slate-900 border border-slate-800 focus:border-emerald-500 rounded-lg text-xs outline-none text-slate-100 font-sans"
                  >
                ...
                <div className="p-3 bg-slate-900/60 border border-slate-800/40 rounded-lg">
                  {/* METRIC SUB FORM */}
                  {activeSlide.visualType === 'metric' && (
                    <div className="space-y-3">
                      <div>
                        <label className="text-[9px] font-mono font-bold text-emerald-400 block uppercase mb-0.5">Nilai Metric (Value)</label>
                        <input
                          type="text"
                          value={activeSlide.metricValue || ''}
                          onChange={(e) => updateSlideField('metricValue', e.target.value)}
                          className="w-full px-2 py-1 bg-slate-950 border border-slate-800 focus:border-emerald-500 rounded text-[11px] outline-none font-mono text-emerald-400 font-bold"
                          placeholder="e.g. +24.0% atau $2.20M USD"
                        />
                      </div>
                      <div>
                        <label className="text-[9px] font-mono font-bold text-slate-400 block uppercase mb-0.5">Label Keterangan Metric</label>
                        <input
                          type="text"
                          value={activeSlide.metricLabel || ''}
                          onChange={(e) => updateSlideField('metricLabel', e.target.value)}
                          className="w-full px-2 py-1 bg-slate-950 border border-slate-800 focus:border-emerald-500 rounded text-[11px] outline-none"
                  ...
                  {activeSlide.visualType === 'bullet_points' && (
                    <div className="space-y-2">
                      <div className="flex justify-between items-center">
                        <label className="text-[9px] font-mono font-bold text-slate-400 block uppercase">Poin-poin Sorotan</label>
                        <button
                          type="button"
                          onClick={handleAddBullet}
                          className="text-[8px] font-sans font-bold text-emerald-400 tracking-wider hover:underline cursor-pointer"
                        >
                          + TAMBAH POIN
                        </button>
                      </div>
                        ...
                        {(activeSlide.bulletsList || []).map((bullet, idx) => (
                          <div key={idx} className="flex items-center gap-1">
                            <input
                              ...
                              className="flex-grow px-2 py-1 bg-slate-950 border border-slate-800 focus:border-emerald-500 rounded text-[10px] outline-none"
                  ...
                  {activeSlide.visualType === 'image' && (
                    <div className="space-y-2">
                      <label className="text-[9px] font-mono font-bold text-slate-400 block uppercase mb-1">Unggah Foto Slide</label>
                      <div className="flex items-center gap-2">
                        {activeSlide.image ? (
                          ...
                          <div className="w-10 h-7 rounded bg-slate-820 border border-dashed border-slate-700 font-mono text-[8px] flex items-center justify-center text-slate-500">
                            NIL
                          </div>
                        )}
                        <input
                        ...
                        <label
                          ...
                          className="px-2.5 py-1 bg-slate-800 hover:bg-slate-700 text-slate-200 text-[10px] font-bold rounded cursor-pointer border border-slate-705 flex items-center gap-1 select-none active:scale-97"
                        >
                  ...
                  {activeSlide.visualType === 'chart' && (
                    <div className="text-[10px] text-slate-400 leading-normal font-mono select-none py-2 text-center">
            ...
            <div className="text-center py-16 text-slate-500 font-mono text-xs border border-dashed border-slate-800 rounded-xl select-none">
